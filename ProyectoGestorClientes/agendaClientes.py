...
def render_clientes():
    ...
    #Se elimina para volver a ordenar
...
def eliminar_cliente():
    #Podemos conocer los elementos seleccionados
    id= tree.selection()[0]
    
    cliente = c.execute("SELECT * FROM cliente WHERE id = ?",(id, )).fetchone()
    respuesta = messagebox.askokcancel('Seguro?','Esta seguro de querer eliminar al cliente: '+cliente[1]+' ?')
    if respuesta:
